...
| P0 | 安装包下载 | 已完成 | 支持 HTTP、HTTPS 和 Mock ZIP data URL，限制 200 MB 与 30 秒超时 |
...
| P0 | 同名目录保护 | 已完成 | 目标目录存在时先返回冲突，用户明确确认后才覆盖 |
| P0 | 前端安装反馈 | 已完成 | 展示安装成功路径、包校验错误、覆盖确认和自动恢复结果 |
| P0 | 基础自动化测试 | 已完成 | 覆盖正常安装、单层目录、路径穿越、目标冲突、确认覆盖、包哈希失败和名称不一致 |
...
| P2 | 同名目录覆盖 | 已完成 | 用户明确确认后才允许覆盖 |
| P2 | 覆盖备份与失败回滚 | 已完成 | 覆盖前保留旧目录，写入失败时自动恢复 |
...
- 本地记录没有可信的平台 `skillId` 时，默认创建新的平台 Skill，首个版本为北京时间当天的 `年.月.日-1`。
- 本地记录能够确认平台来源时，默认发布到对应 Skill；版本号按北京时间日期自动生成，同一天递增末尾序号。
...
- 不跟随 Skill 目录内部指向目录外部的软连接；遇到越界路径、循环链接、不可读文件或超过 200 MB 时停止上传并给出具体原因。
...
- 未关联平台的本地 Skill 可以创建当天日期首版；已关联平台的本地 Skill 可以发布当天的下一个日期版本。
